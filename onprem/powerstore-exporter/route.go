...
func Run(config *utils.Config, logger log.Logger) {
	...
	for _, storage := range config.StorageList {
		...
		AlertRegistry := prometheus.NewPedanticRegistry()
		...
		AlertRegistry.MustRegister(generalCollector.NewAlertCollector(client, logger))
		...
		metricsGroup := r.Group(fmt.Sprintf("/metrics/%s", storage.Ip))
		{
			...
			metricsGroup.GET("alert", utils.PrometheusHandler(AlertRegistry, logger))
		}
